...
def update_lr(
    optimizer, initial_lr, update, num_updates, use_linear_lr_decay, freeze_lr_n_updates
):
...
def set_up_experiment_folder(folder, ckpt_idx, use_checkpoint=False):
    ...
    if use_checkpoint:
        if ckpt_idx != -1:
            assert (
                ckpt_idx in all_ckpt_indices
            ), "ckpt idx requested {} does not exist".format(ckpt_idx)
    ...
    if os.path.isfile(log_file) and ckpt_idx != -1:
        with open(log_file) as f:
            lines = [
                ...
                if ("update: {}".format(ckpt_idx)) in line
            ]
            if len(lines) > 0:
                tokens = lines[0].split("\t")
                if (
                    len(tokens) > 1
                    and "env_step" in tokens[1]
                    and len(tokens[1].split()) > 1
                ):
                    try:
        ...
        os.rename(
            ...
            log_file
            + "_"
            + datetime.fromtimestamp(os.path.getctime(log_file)).strftime(
                "%Y-%m-%d_%H:%M:%S"
            ),
        )
