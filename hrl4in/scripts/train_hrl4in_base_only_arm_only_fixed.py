...
def evaluate(
    ...
    while episode_counts.sum() < args.num_eval_episodes:
        with torch.no_grad():
            ...
            robot_pos_cpu = robot_pos.clone()
            robot_pos_cpu = robot_pos_cpu.cpu().numpy()  
            
            temp_current_subgoals_cpu = temp_current_subgoals.clone()
            temp_current_subgoals_cpu = temp_current_subgoals_cpu.cpu().numpy()
...
def main():
    ...
    for sensor in rollouts.observations:
        if (sensor in batch):  # because we will not have subgoal, subgoal_mask and action_mask in the batch_obs
    ...
    for update in range(start_epoch, args.num_updates):
        ...
        for step in range(args.num_steps):
            ...
            with torch.no_grad():
                ...
                (
                    ...
                    meta_step_observation,
                    meta_rollouts.recurrent_hidden_states[
                        meta_rollouts.valid_steps, meta_rollouts.env_indices
                    ],
                    meta_rollouts.masks[
                        meta_rollouts.valid_steps, meta_rollouts.env_indices
                    ],
                )
                # action_mask_indices -> which embodiement to use. Shape = (num_envs, 1). 
                # action_mask_indices -> Example: torch.tensor([0,1,0,1])
                # In index_select the second argument can have any length, just the value it contains should be in the index range 
                ...
                current_subgoal_log_probs = (
                    should_use_new_subgoals * subgoal_log_probs
                ...
                current_action_mask_indices = (
                    should_use_new_subgoals.long() * action_mask_indices
                    + (1 - should_use_new_subgoals.long()) * current_action_mask_indices
                )
                current_action_mask_log_probs = (
                    should_use_new_subgoals * action_mask_log_probs
                ...
                temp_current_subgoals = current_subgoals.clone()
                temp_current_subgoals = temp_current_subgoals.view(-1, 2, 3)
                temp_current_action_mask_indices = current_action_mask_indices.clone()
                temp_current_action_mask_indices = torch.cat([1 - temp_current_action_mask_indices, temp_current_action_mask_indices], 1)
                temp_current_action_mask_indices = temp_current_action_mask_indices.unsqueeze(-1)

                temp_current_subgoals = temp_current_action_mask_indices*temp_current_subgoals
                temp_current_subgoals = torch.sum(temp_current_subgoals, 1)
                ...
                rollouts.observations["auxiliary_sensor"][step][:, 87:90] = local_subgoal
                rollouts.observations["auxiliary_sensor"][step][:, 90:93] = temp_current_subgoals
                ...
                base_step_observation = step_observation.copy()
                base_step_observation["sensor"] = base_step_observation["sensor"][:, 0:3]
                arm_step_observation = step_observation.copy()
                arm_step_observation["sensor"] = arm_step_observation["sensor"][:, 3:]
                ...
                (
                    ...
                    arm_step_observation,
                    rollouts.recurrent_hidden_states[step],
                    rollouts.masks[step],
                    update=update,
                )
                values = (1 - current_action_mask_indices)*base_values + current_action_mask_indices*arm_values
                actions = (1 - current_action_mask_indices)*base_actions + current_action_mask_indices*arm_actions
                action_log_probs = (1 - current_action_mask_indices)*base_action_log_probs + current_action_mask_indices*arm_action_log_probs
                recurrent_hidden_states = (1 - current_action_mask_indices)*base_recurrent_hidden_states+ current_action_mask_indices*arm_recurrent_hidden_states
            ...
            penalty_prob_th = 0.5
            current_subgoals_penalty = (
                args.subgoal_failed_penalty
                * subgoals_done
                * (1.0 - subgoals_achieved)
                * (
                    torch.rand(train_envs._num_envs, 1, device=device) < penalty_prob_th
                ).float()
            )
            ...
            if args.use_action_hindsight:
                hindsight_subgoals = (
                    next_obs_batch["sensor"] - meta_step_observation["sensor"]
                )
                (
                    _,
                    ...
                    _,
                    _,
                ) = meta_actor_critic.evaluate_actions(
                    meta_step_observation,
                    meta_rollouts.recurrent_hidden_states[
                        meta_rollouts.valid_steps, meta_rollouts.env_indices
                    ],
                ...
                hindsight_subgoals = (
                    subgoals_achieved * original_subgoals
                    + (1 - subgoals_achieved) * hindsight_subgoals
                )
                hindsight_subgoal_log_probs = (
                    subgoals_achieved * current_subgoal_log_probs
            ...
            intrinsic_reward += subgoals_achieved.float() * args.subgoal_achieved_reward
            intrinsic_reward += (
                collision_rewards * args.extrinsic_collision_reward_weight
            )
            intrinsic_reward += rewards * args.extrinsic_reward_weight
        ...
        with torch.no_grad():
            ...
            local_subgoal = torch.tensor(local_subgoal).cuda()
            last_observation["auxiliary_sensor"][:, 87:90] = local_subgoal
            last_observation["auxiliary_sensor"][:, 90:93] = temp_current_subgoals
            ...
            base_last_observation = last_observation.copy()
            base_last_observation["sensor"] = base_last_observation["sensor"][:, 0:3]
            arm_last_observation = last_observation.copy()
            arm_last_observation["sensor"] = arm_last_observation["sensor"][:, 3:]

            next_base_value = base_actor_critic.get_value(
                base_last_observation,
                rollouts.recurrent_hidden_states[-1],
                rollouts.masks[-1],
            ).detach()
            next_arm_value = arm_actor_critic.get_value(
                arm_last_observation,
                rollouts.recurrent_hidden_states[-1],
                rollouts.masks[-1],
            ).detach()
            next_value = (1 - current_action_mask_indices)*next_base_value + current_action_mask_indices*next_arm_value
        ...
        if update > 0 and update % args.log_interval == 0:
            ...
            writer.add_scalar(
                "losses/meta_value_loss", meta_value_loss, global_step=update
            )
            writer.add_scalar("losses/subgoal_loss", subgoal_loss, global_step=update)
            writer.add_scalar(
                "losses/meta_dist_entropy", meta_dist_entropy, global_step=update
            )
            ...
            window_rewards = (
                window_subgoal_reward[-1] - window_subgoal_reward[0]
            ).sum()
            window_success_rates = (
                window_subgoal_success_rates[-1] - window_subgoal_success_rates[0]
            ).sum()
            window_lengths = (
                window_subgoal_lengths[-1] - window_subgoal_lengths[0]
            ).sum()
            ...
            if window_counts > 0:
                ...
                writer.add_scalar(
                    "train/updates/subgoal_reward", reward_mean, global_step=update
                )
                writer.add_scalar(
                    "train/updates/subgoal_success_rate",
                    success_rate_mean,
                    global_step=update,
                )
                writer.add_scalar(
                    "train/updates/subgoal_length", lengths_mean, global_step=update
                )
                writer.add_scalar(
                    "train/env_steps/subgoal_reward",
                    reward_mean,
                    global_step=count_steps,
                )
                writer.add_scalar(
                    "train/env_steps/subgoal_success_rate",
                    success_rate_mean,
                    global_step=count_steps,
                )
                writer.add_scalar(
                    "train/env_steps/subgoal_length",
                    lengths_mean,
                    global_step=count_steps,
                )
            else:
