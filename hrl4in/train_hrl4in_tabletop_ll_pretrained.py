...
def evaluate(
    ...
    current_subgoals = torch.zeros(batch["task_obs"].shape, device=device)
    current_subgoals_steps = torch.zeros(envs._num_envs, 1, device=device)
    current_subgoal_masks = torch.zeros(batch["task_obs"].shape, device=device)
    ...
    while episode_counts.sum() < args.num_eval_episodes:
        with torch.no_grad():
            ...
            ideal_next_state = batch["sensor"][:, :3] + current_subgoals
        ...
        subgoals_diff = (
            ideal_next_state - next_obs_batch["sensor"][:, :3]
        ) * current_subgoal_masks
        ...
        prev_potential = ideal_next_state - prev_batch["sensor"][:, :3]
        prev_potential = torch.norm(
            prev_potential * current_subgoal_masks, dim=1, keepdim=True
        )

        current_potential = ideal_next_state - next_obs_batch["sensor"][:, :3]
...
def main():
    ...
    original_subgoals = torch.zeros(batch["task_obs"].shape, device=device)
    ...
    for update in range(start_epoch, args.num_updates):
        ...
        for step in range(args.num_steps):
            ...
            with torch.no_grad():
                ...
                ideal_next_state = step_observation["sensor"][:, :3] + current_subgoals
            ...
            prev_potential = ideal_next_state - step_observation["sensor"][:, :3]
            prev_potential = torch.norm(
                prev_potential * current_subgoal_masks, dim=1, keepdim=True
            )
            current_potential = ideal_next_state - next_obs_batch["sensor"][:, :3]
            ...
            current_subgoals = (
                ideal_next_state[:, :3] - next_obs_batch["sensor"][:, :3]
            ) * current_subgoal_masks
            current_subgoals_steps = (1 - subgoals_done) * current_subgoals_steps
