...
def evaluate(
    ...
    while episode_counts.sum() < args.num_eval_episodes:
        ...
        subgoals_diff = (
            ideal_next_state - next_obs_batch["arm_world"]
        ) * current_subgoal_masks
    ...
    if eval_only:
        ...
        writer.add_scalar(
            "eval/env_steps/spl", episode_spl_mean, global_step=count_steps
        )
        writer.add_scalar(
...
def main():
    ...
    config_file = os.path.join(
        os.path.dirname(gibson2.__file__), "../examples/configs", args.config_file
    )
    ...
    if model_ids is None:
        ...
        assert (
            len(model_ids) == args.num_train_processes
    ...
    if model_ids_eval is None:
        ...
        assert (
            len(model_ids_eval) == args.num_eval_processes
        ), "model ids provided, but length not equal to num_train_processes"
    ...
    train_envs = [
        lambda device_idx=sim_gpu_id[env_id_to_which_gpu[env_id]], model_id=model_ids[
            env_id
    ...
    eval_envs = [
        lambda device_idx=sim_gpu_id[
            env_id_to_which_gpu[env_id]
        ], model_id=model_ids_eval[env_id - args.num_train_processes]: load_env(
            model_ids[0], "headless", device_idx
        )
        for env_id in range(
    ...
    eval_envs += [
        lambda: load_env(
            model_ids_eval[-1], args.env_mode, sim_gpu_id[env_id_to_which_gpu[-1]]
        )
    ]
    ...
    for update in range(start_epoch, args.num_updates):
        ...
        for step in range(args.num_steps):
            ...
            with torch.no_grad():
                ...
                ideal_next_state = (
                    step_observation["arm_world"] + current_subgoals_in_global_frame
                )
        ...
        if update > 0 and update % args.log_interval == 0:
            ...
            if window_counts > 0:
                ...
                writer.add_scalar(
                    "train/env_steps/spl", spl_mean, global_step=count_steps
                )
                writer.add_scalar(
